...
if (ascs.utility.enableGenerator()) {
    console.log('Using ES6 mode');
    ...
    describe("basic-api-scope", function (){
        ...
        describe("ascs.conv", function ()	{
            ...
            it("the functions can be converted to a ascs.func Object", function (done) {
            ...
            it("the non-function should return null", function (done) {
            ...
            it("the asc.func object should return itself", function (done) {
        ...
        describe("ascs.await", function ()	{
            ...
            it("the await operation can block the env-function", function (done) {
                ...
                ascs.env(function*()    {
                    ...
                    check_flag = 200;
                    check_flag = 300;
                    yield (async_f2("Hello", "world")); // this callback will be executed immediately.
                    check_flag = 400;                       // when the 'await' called, the status has changed COMPLETE. `await` canceled
                })();
                assert.equal(check_flag, 300, "this step 3, the check flag must be 400 if the await has block the process<jump out the env>");
    
                ascs.env(function*()    {
                    yield (async_f3("Superhero"));
                    check_flag = 1000;
                    check_flag = 2000;
                    check_flag = 3000;
                })();
                ...
                ascs.env(function*()    {
                    check_flag = 10000;
                    check_flag = 20000;
                    yield (async_f4()); // the same of async_f2, the callback will execute immediately
                    check_flag = 30000;
                })();
                assert.equal(check_flag, 20000, "this step 5, the check flag must be 20000 if the await has block the process<jump out the env>");
                done();    
            });
            ...
            it("after await, all the result are returned successfuly", function (done) {    // use mocha's asynchronous supported
                ascs.env(function*() {
                    ...
                    result.push(yield (t1))
                    result.push(yield (t2))
                    result.push(yield (t3))
                    result.push(yield (t4))
                    result.push(yield (t5))
            ...
            it("if no await operation, the env-function will be execute asynchronous", function (done) {
                var flag = [];
                var i = 1;
                ascs.env(function*() {
                    flag.push(i++);
            ...
            it("the time-cost between the normal asynchronous callback and ascs.await model", function (done) {
                ...
                */
                ascs.env(function *() {
                    ...
                    var result1 = yield (millionFib_async());	// waiting for callback
