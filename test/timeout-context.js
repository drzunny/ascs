    ...
    describe("timeout-context-scope", function () {
        it("await-timeout, try to break the await", function (done) {
            ...
            ascs.env(function *() {
                ...
                function fun ()    {
                    this.fun3 = function fun3 (a,b,cb) {
                        setTimeout(function () { cb(a+b); }, 250);
                    };
                };
                fun.fun4 = function fun4 (s, cb) {
                ...
                yield t1.timeout(100);
                assert.equal(t1.status().code, ascs.utility.TASK_STATE.complete, "the fs.exists should be done");
                var t2 = fun2_async();
                yield t2.timeout(40);
                assert.equal(t2.status().code, ascs.utility.TASK_STATE.timeout, "the fun2 should be TIMEOUT here");
                var t3 = fun3_async(1,2);
                yield t3.timeout(200);
                assert.equal(t3.status().code, ascs.utility.TASK_STATE.timeout, "the fun3 should be TIMEOUT here");
                var t4 = fun4_async('Fuckit');
                yield t4.timeout(760);
                assert.equal(t4.status().code, ascs.utility.TASK_STATE.complete, "the fun4 should be COMPLETE here");
                yield t5.timeout(700);                  // before this await, the fun5_async has run 1100ms (total 1500ms)
                assert.equal(t5.status().code, ascs.utility.TASK_STATE.complete, "the fun5 should be COMPLETE here, and it has been done before this time");
        ...
        it("context-changed, changed the function's binding object", function (done) {
                ...
                ascs.env(function*() {
                    ...
                    //---------------------------
                    var rs1_owner = yield f1();
                    var rs2_owner = yield f2();
                    var rs2_owner_ = yield f2_();
                    var rs3_owner = yield f3();
                    var rs4_owner = yield f4();
                    var rs5_owner = yield f5();
